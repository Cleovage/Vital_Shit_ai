...
class MetricDetailViewModel @Inject constructor(
    ...
    private fun buildDetail(
        ...
        val todayChartValues = todaySeries(metric, snapshot)
        ...
        return MetricDetailData(
            ...
            todayChartValues = todayChartValues,
    ...
    private fun todaySeries(metric: HealthMetricType, snapshot: HealthSnapshot): List<Float> {
        return when (metric) {
            HealthMetricType.STEPS -> snapshot.hourlySteps.toSortedMap().values.map { it.toFloat() }.ifEmpty { listOf(0f, snapshot.steps.toFloat()) }
            HealthMetricType.HEART_RATE -> snapshot.hourlyHeartRate.toSortedMap().values.map { it.toFloat() }.ifEmpty { listOf(0f, snapshot.avgHeartRate.toFloat()) }
            HealthMetricType.ACTIVE_CALORIES -> listOf(0f, snapshot.calories.toFloat())
            HealthMetricType.SLEEP -> listOf(0f, snapshot.sleepDurationHours.toFloat())
            HealthMetricType.DISTANCE -> listOf(0f, (snapshot.distanceMeters / 1000.0).toFloat())
            HealthMetricType.HYDRATION -> listOf(0f, snapshot.hydrationLiters.toFloat())
            HealthMetricType.EXERCISE_MINUTES -> listOf(0f, snapshot.exerciseMinutes.toFloat())
            HealthMetricType.CALORIES_INTAKE -> listOf(0f, snapshot.caloriesIntake.toFloat())
            HealthMetricType.PROTEIN -> listOf(0f, snapshot.proteinGrams.toFloat())
            HealthMetricType.CARBOHYDRATE -> listOf(0f, snapshot.carbsGrams.toFloat())
            HealthMetricType.FAT -> listOf(0f, snapshot.fatGrams.toFloat())
            else -> listOf(0f, 0f)
        }
    }
